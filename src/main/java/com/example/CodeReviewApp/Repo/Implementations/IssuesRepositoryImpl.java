...
public class IssuesRepositoryImpl implements IssuesRepository{
    ...
    public void insert(List<SonarIssue> issues,Long submissionId) {
        ...
        var queries = issues.stream().map(issue ->  dsl.insertInto(ISSUES).values(ISSUES.SUBMISSION_ID,ISSUES.SEVERITY,ISSUES.LINE_NUMBER,ISSUES.FILE_NAME,ISSUES.MESSAGE)
        .values(submissionId,issue.getSeverity(),issue.getLine(),issue.getComponent(),issue.getMessage())
        ).toList();
